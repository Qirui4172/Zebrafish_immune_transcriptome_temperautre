...
out.plot <- paste0("Plots_", comp, "_rna.pdf")
pdf(out.plot)
...
print(ggbiplot(pca.results3, obs.scale = 1, var.scale = 1, groups = group$Treatment, ellipse = TRUE, var.axes = FALSE))
...
if (comptype == "LPS") {
    ...
    dds24 <- dds24[rowSums(counts(dds24)) >= 3, ]
    vst24 <- vst(dds24, blind = FALSE)
    dds28 <- dds[, dds$Temperature == 28]
    dds28 <- dds28[rowSums(counts(dds28)) >= 3, ]
    vst28 <- vst(dds28, blind = FALSE)
    dds32 <- dds[, dds$Temperature == 32]
    dds32 <- dds32[rowSums(counts(dds32)) >= 3, ]
    vst32 <- vst(dds32, blind = FALSE)
} else if (comptype == "Temp") {
...
AddGeneName <- function(res, compString) {
    ...
    genemap <- getBM(attributes = c("ensembl_gene_id", "entrezgene_id", "zfin_id_symbol"), 
    ...
    out.deg <- paste("DEGs_", compString, ".tsv", sep = "")
...
VolcanoPlot <- function(res, compString, fc, fc.low, fc.high, padj.max, ratio) {
    ...
    volcano[which(volcano$padj >= padj.max & volcano$padj < 0.05 & 
		volcano$log2FoldChange >= fc.low & volcano$log2FoldChange <= -log2(fc)), 8] <- rep(1)

    # group2: up-regulated DEGs within normal fc and padj ranges (red dots)
    volcano[which(volcano$padj >= padj.max & volcano$padj < 0.05 & 
		volcano$log2FoldChange >= log2(fc) & volcano$log2FoldChange <= fc.high), 8] <- rep(2)
    ...
    volcano[which(volcano$padj < padj.max & volcano$log2FoldChange < fc.low), 
		c("log2FoldChange", "padj", "group")] <- list(log2FoldChange = fc.low, padj = padj.max, group = 5)
    ...
    volcano[which(volcano$padj < padj.max & volcano$log2FoldChange > fc.high), 
		c("log2FoldChange", "padj", "group")] <- list(log2FoldChange = fc.high, padj = padj.max, group = 6)
...
if (comptype == "LPS") {
    ...
    pheatmap(vst.deg1.5, 
		main = "Heatmap of LPSvsCT DEGs fc>1.5", 
		scale = "row", 
    ...
    pheatmap(vst.deg2, 
		main = "Heatmap of LPSvsCT DEGs fc>2", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
    ...
    pheatmap(vst.deg1.5, 
		main = "Heatmap of 24/32_vs_28 DEGs fc>1.5", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
    ...
    pheatmap(vst.deg2, 
		main = "Heatmap of 24/32_vs_28 DEGs fc>2", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
