...
# Read files
demir.group <- read.delim(args[1], stringsAsFactors = FALSE, check.names = FALSE)
mirna.target <- read.delim(args[2], header = FALSE, stringsAsFactors = FALSE, check.names = FALSE)
mirna.target <- mirna.target[, 1:2, drop = FALSE]
colnames(mirna.target) <- c("miRNA", "Ensembl")
sample.info <- read.delim(args[3], stringsAsFactors = FALSE, check.names = FALSE)
gene.mx <- read.delim(args[4], row.names = 1, check.names = FALSE)
mirna.mx <- read.delim(args[5], row.names = 1, check.names = FALSE)

if (!all(c("miRNA", "Group", "KeyTemperature", "KeyTreatment") %in% names(demir.group))) {
	stop("The DEmiRNA group file is missing required columns.")
}
if (!all(c("Sample", "Temperature", "Treatment") %in% names(sample.info))) {
	stop("The sample information file must contain Sample, Temperature, and Treatment columns.")
}
sample.info <- sample.info[sample.info$Sample %in% colnames(gene.mx) & sample.info$Sample %in% colnames(mirna.mx), , drop = FALSE]
...
for(i in 1:nrow(demir.group)) {
	...
	target.ids <- unique(mirna.target$Ensembl[mirna.target$miRNA == mir])
	target.num <- sum(target.ids %in% rownames(gene.mx))
...
for(i in 1:nrow(demir.group)) {
	...
	if (keytreatment == "Control") {
		...
		warning("Skipping unsupported treatment: ", keytreatment)
		next
	}

	# isolate subset of miRNA and gene matrix
	if (!mir %in% rownames(mirna.mx)) {
		warning("Skipping miRNA absent from the miRNA matrix: ", mir)
		next
	}
	mirna_mx.sub <- mirna.mx[mir, sample.names, drop = FALSE]
	target.id <- unique(mirna.target$Ensembl[mirna.target$miRNA == mir])
	target.id <- intersect(target.id, rownames(gene.mx))
	gene_mx.sub <- gene.mx[target.id, sample.names, drop = FALSE]
	mir_for_cor <- t(mirna_mx.sub)

	if(nrow(gene_mx.sub) == 0) {
		next
	} # some miRNAs may have no target genes, and these miRNAs are supposed to be excluded beforehand but it still could be input by mistake (DEmiRNA_group, DEmiRNA_targetGene)!!
	...
	for(j in 1:nrow(gene_mx.sub)) {
		k <- k + 1
		if(sum(is.na(gene_mx.sub[j, ])) != 0 | sd(gene_mx.sub[j, ]) == 0) {
			coeff = NA
			pvalue = NA
			cor.mx[k,] = c(coeff, pvalue)
			next
		} # check target genes and sd values, some predicted target genes may be not in gene.matrix file or some genes may have sd==0
