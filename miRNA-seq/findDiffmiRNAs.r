...
comptype <- args[4]
if (!comptype %in% c("LPS", "Temp")) {
    stop("[compaType] must be either 'LPS' or 'Temp'.")
}

nonzero <- suppressWarnings(as.numeric(args[5]))
rowsum <- suppressWarnings(as.numeric(args[6]))
if (is.na(nonzero) || nonzero < 1 || nonzero > 5 || nonzero != as.integer(nonzero)) {
    stop("[minNonzero] must be an integer from 1 to 5.")
}
if (is.na(rowsum) || rowsum < 1) {
    stop("[minRowsum] must be a positive number.")
}
...
cat("Read data and parameters\n\n")
readcounts <- read.delim(args[1], row.names = 1, check.names = FALSE)
readcounts <- as.matrix(readcounts)
storage.mode(readcounts) <- "numeric"
sampleinfo.origin <- read.delim(args[2], stringsAsFactors = FALSE, check.names = FALSE)
if (anyDuplicated(sampleinfo.origin$Sample)) {
    stop("Sample names in the metadata must be unique.")
}
sample.order <- match(colnames(readcounts), sampleinfo.origin$Sample)
if (anyNA(sample.order)) {
    stop("Every count-matrix column must have a matching metadata Sample entry.")
}
sampleinfo.origin <- sampleinfo.origin[sample.order, , drop = FALSE]
sampleinfo <- sampleinfo.origin
rownames(sampleinfo) <- sampleinfo$Sample
sampleinfo$Treatment <- factor(sampleinfo$Treatment)
sampleinfo$Temperature <- factor(sampleinfo$Temperature)

rmsample <- args[3]

if (rmsample != "no" && !all(rmsample %in% sampleinfo$Sample)) {
    stop("Every sample in removeSamp must exist in the metadata.")
}

comp <- if (comptype == "LPS") "LPSvsCT" else "ct2432vs28"
out.plot <- paste0("Plots_", comp, "_mirna.pdf")
pdf(out.plot)

cat("==========================================================================================\n")
## Generate metadata
cat("Generate metadata\n\n")
...
data1 <- pca.result1[[2]]
print(pca.plot1 + coord_fixed(ratio = 5/3))
print(pca.plot1 + coord_fixed(ratio = 5/3) + geom_text(aes(label = rownames(data1)), size = 1, hjust = -0.4))
print(pca.plot1 + coord_fixed(ratio = 5/3) + geom_text(aes(label = rownames(data1)), size = 1, vjust = -1.8))
...
PCA_inhouse <- function(vst, sampleinfo) {
    ...
    pca.plot <- ggplot(data, aes(PC1, PC2, colour = Temperature, shape = Treatment)) + 
		geom_point(size = 3) + 
...
data2 <- pca.result2[[2]]
print(pca.plot2)
print(pca.plot2 + geom_text(aes(label = rownames(data2)), size = 1, hjust = -0.4))
print(pca.plot2 + geom_text(aes(label = rownames(data2)), size = 1, vjust = -1.8))

# use 'ggbiplot' package
pca.results3 <- prcomp(t(as.data.frame(assay(vst))), scale = TRUE)
group <- as.data.frame(colData(vst))
if (requireNamespace("ggbiplot", quietly = TRUE)) {
    print(ggbiplot::ggbiplot(pca.results3, obs.scale = 1, var.scale = 1, groups = group$Treatment, ellipse = TRUE, var.axes = FALSE))
} else {
    warning("Skipping ggbiplot: package is not installed.")
}

cat("==========================================================================================\n")
# Correlation & clustering
cat("Correlating samples and clustering...\n\n")
# use 'GGally' package library(GGally)
...
corrCluster <- function(vst) {
    ...
    correlation.p <- pheatmap(as.data.frame(sampleDist.mx), 
...
if ((length(rmsample) == 1 && rmsample != "no") || length(rmsample) > 1) {
    for (i in rmsample) {
        dds <- dds[, dds$Sample != i]
        sampleinfo <- sampleinfo[which(sampleinfo$Sample != i), ]
    }
}
dds <- dds[rowSums(counts(dds)) >= 100, ]
vst <- varianceStabilizingTransformation(dds, blind = FALSE)
...
pca.plot1 <- pca.result1[[1]]
data1 <- pca.result1[[2]]
print(pca.plot1 + coord_fixed(ratio = 1))
print(pca.plot1 + coord_fixed(ratio = 1) + geom_text(aes(label = rownames(data1)), size = 1, hjust = -0.4))
print(pca.plot1 + coord_fixed(ratio = 1) + geom_text(aes(label = rownames(data1)), size = 1, vjust = -1.8))
...
remove_lowcount_miRNAs <- function(dds, nonzero, rowsum, cols1, cols2) {
    ...
    for (i in rownames(dds.df)) {
        if ((sum(dds.df[i, cols1]) >= sum(dds.df[i, cols2]) && sum(dds.df[i, cols1] >= 1) >= nonzero && rowSums(dds.df[i, c(cols1, cols2)]) >= rowsum) || 
			(sum(dds.df[i, cols2]) >= sum(dds.df[i, cols1]) && sum(dds.df[i, cols2] >= 1) >= nonzero && rowSums(dds.df[i, c(cols1, cols2)]) >= rowsum)) {
            dds.df[i, "keep"] <- "yes"
        } else {
            dds.df[i, "keep"] <- "no"
        }
...
if (comptype == "LPS") {
    ...
    write.table(as.data.frame(res24.fc1.5), "DEmiRNAs_24lpsvsct.tsv", row.names = T, col.names = T, quote = F, sep = "\t")
    ...
    write.table(as.data.frame(res28.fc1.5), "DEmiRNAs_28lpsvsct.tsv", row.names = T, col.names = T, quote = F, sep = "\t")
    ...
    write.table(as.data.frame(res32.fc1.5), "DEmiRNAs_32lpsvsct.tsv", row.names = T, col.names = T, quote = F, sep = "\t")
} else if (comptype == "Temp") {
    ...
    write.table(as.data.frame(res24vs28.fc1.5), "DEmiRNAs_24vs28.tsv", row.names = T, col.names = T, quote = F, sep = "\t")
    ...
    write.table(as.data.frame(res32vs28.fc1.5), "DEmiRNAs_32vs28.tsv", row.names = T, col.names = T, quote = F, sep = "\t")
}
...
VolcanoPlot <- function(res, compString, fc, fc.low, fc.high, padj.max, ratio) {
    ...
    volcano$significant <- as.factor(
		ifelse(!is.na(volcano$padj) & volcano$padj < 0.05 & abs(volcano$log2FoldChange) > log2(fc),
			    ifelse(volcano$log2FoldChange > log2(fc), "Up", "Down"), 
				"No"))
    ...
    volcano[which(volcano$padj >= padj.max & volcano$padj < 0.05 & volcano$log2FoldChange >= fc.low & 
		volcano$log2FoldChange <= -log2(fc)), 8] <- rep(1)

    # group2: up-regulated DEmiRNAs within normal fc and padj ranges (red dots)
    volcano[which(volcano$padj >= padj.max & volcano$padj < 0.05 & volcano$log2FoldChange >= log2(fc) & 
		volcano$log2FoldChange <= fc.high), 8] <- rep(2)

    # group3: non-significant genes within normal fc or padj ranges (grey dots)
    volcano[which((volcano$padj >= 0.05 & abs(volcano$log2FoldChange) <= fc.high) | 
		(volcano$padj < 0.05 & volcano$padj > padj.max & abs(volcano$log2FoldChange) <= log2(fc))), 8] <- rep(3)

    # group4: non-significant genes outside of normal fc or padj ranges (grey trangle)
    volcano[which(volcano$padj >= 0.05 & volcano$log2FoldChange < fc.low), c("log2FoldChange", "group")] <- 
		list(log2FoldChange = fc.low, group = 4)
    volcano[which(volcano$padj >= 0.05 & volcano$log2FoldChange > fc.high), c("log2FoldChange", "group")] <- 
		list(log2FoldChange = fc.high, group = 4)
    volcano[which(volcano$padj < padj.max & abs(volcano$log2FoldChange) < -log2(fc)), c("padj", "group")] <- 
		list(padj = padj.max, group = 4)

    # group5: down-regulated genes outside of normal fc or padj ranges (blue trangle)
    volcano[which(volcano$padj >= padj.max & volcano$padj < 0.05 & volcano$log2FoldChange < fc.low), 
		c("log2FoldChange", "group")] <- list(log2FoldChange = fc.low, group = 5)
    volcano[which(volcano$padj < padj.max & volcano$log2FoldChange < fc.low), c("log2FoldChange", "padj", "group")] <- 
		list(log2FoldChange = fc.low, padj = padj.max, group = 5)
    volcano[which(volcano$padj < padj.max & volcano$log2FoldChange <= -log2(fc) & volcano$log2FoldChange >= fc.low), 
		c("padj", "group")] <- list(padj = padj.max, group = 5)

    # group6: up-regulated genes outside of normal fc or padj ranges (red trangle)
    volcano[which(volcano$padj >= padj.max & volcano$padj < 0.05 & volcano$log2FoldChange > fc.high), 
		c("log2FoldChange", "group")] <- list(log2FoldChange = fc.high, group = 6)
    volcano[which(volcano$padj < padj.max & volcano$log2FoldChange > fc.high), c("log2FoldChange", "padj", "group")] <- 
		list(log2FoldChange = fc.high, padj = padj.max, group = 6)
    volcano[which(volcano$padj < padj.max & volcano$log2FoldChange >= log2(fc) & volcano$log2FoldChange <= fc.high), 
		c("padj", "group")] <- list(padj = padj.max, group = 6)

    # plot
    title <- paste("Volcano plot of ", compString, " foldchange>", fc, sep = "")
    volcano.p <- ggplot(volcano, aes(log2FoldChange, -log10(padj))) + 
		geom_point(data = volcano[which(volcano$group == 1), ], color = "royalblue3", alpha = 0.8) + 
		geom_point(data = volcano[which(volcano$group == 2), ], color = brewer.pal(11, "RdYlBu")[2], alpha = 0.8) + 
		geom_point(data = volcano[which(volcano$group == 3), ], color = "gray60", alpha = 0.8) +
        geom_point(data = volcano[which(volcano$group == 4), ], shape = 2, color = "gray60", alpha = 0.8) + 
		geom_point(data = volcano[which(volcano$group == 5), ], shape = 2, color = "royalblue3", alpha = 0.8) + 
		geom_point(data = volcano[which(volcano$group == 6), ], shape = 2, color = brewer.pal(11, "RdYlBu")[2], alpha = 0.8)
	
    volcano.p <- volcano.p + 
		labs(title = {title}, x = "log2FoldChange", y = "-log10(padj)") + 
		geom_hline(yintercept = -log10(0.05), linetype = 2, size = 0.3, color = "gray60") +
        geom_vline(xintercept = c(-log2(fc), log2(fc)), linetype = 2, size = 0.3, color = "gray60") + 
		xlim({fc.low}, {fc.high}) + 
		ylim(0, {y.max}) + 
		theme_bw() + 
...
if (comptype == "LPS") {
    anno.label <- c(rep("24 C control", 5), 
		rep("24 C LPS", 5), 
		rep("28 C control", 5), 
		rep("28 C LPS", 5), 
		rep("32 C control", 5), 
    ...
    pheatmap(vst.demir1.5, 
		main = "Heatmap of LPSvsCT DEmiRNAs fc>1.5", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
		show_rownames = F,
    ...
    pheatmap(vst.demir1.5, 
		main = "Heatmap of LPSvsCT DEmiRNAs fc>1.5", 
		scale = "row", color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = TRUE, 
		treeheight_col = 20,
        show_rownames = F, 
    ...
    pheatmap(vst.demir2, 
		main = "Heatmap of LPSvsCT DEmiRNAs fc>2", scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
		show_rownames = F,
    ...
    pheatmap(vst.demir2, 
		main = "Heatmap of LPSvsCT DEmiRNAs fc>2", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = TRUE, 
    ...
    pheatmap(vst.demir1.5, 
		main = "Heatmap of 24/32_vs_28 DEmiRNAs fc>1.5", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
    ...
    pheatmap(vst.demir2, 
		main = "Heatmap of 24/32_vs_28 DEmiRNAs fc>2", 
		scale = "row", 
		color = colorRampPalette(c(rev(brewer.pal(9, "Blues")[c(3:9)]), brewer.pal(9, "OrRd")[c(3:9)]))(100), 
		cluster_rows = TRUE, 
		cluster_cols = FALSE, 
